...
def solve_problem(problem_content: str) -> ProblemSolution:
    """Return a deterministic mock solution for the solve-flow MVP."""

    # Keep the input in the service contract so a real provider can replace this
    # implementation without changing the API layer.
    _ = problem_content
    ...
    return ProblemSolution(
        ...
        solution_approach=(
            "从左到右遍历数组。对当前元素 value，计算还需要的数 complement = target - value。"
            "如果 complement 已经出现在哈希表中，就返回它的下标和当前下标；"
            "否则把当前值及其下标存入哈希表。"
        ),
        algorithm_reason=(
            "暴力枚举每一对元素需要 O(n²) 时间。哈希表能够用接近 O(1) 的平均时间判断"
            "补数是否出现，从而把一次嵌套查找改成一次线性遍历。"
        ),
        python_code=(
