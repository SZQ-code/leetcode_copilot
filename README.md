...
当前仓库已经打通可持久化的 AI 学习闭环：前端可以提交题目文本，后端通过可配置的 AI Provider 生成结构化解析并保存到 SQLite。用户可以浏览历史记录、查看详情、保存掌握状态和个人备注，并按标签查看薄弱题型与待复习队列。题目详情页还提供单 Agent 题目导师，能够调用学习工具、保存长期记忆并展示工具轨迹。未配置 DeepSeek API Key 时，系统自动使用确定性 Mock。
...
1. 打开任意已保存题目的详情页。
2. 在桌面端右侧“题目导师”面板发送问题；移动端切换到“题目导师”标签。
3. 首次对话会调用 `get_problem_context`，工具轨迹会显示在消息下方。
4. Agent 可以查询相似题和学习画像，也可以保存误区、掌握点与复习重点。
5. Agent 提议修改掌握状态或备注时，必须点击“确认执行”才会真正更新。

每道题保留一个持续会话，刷新页面后对话、工具轨迹和学习记忆仍然存在。
